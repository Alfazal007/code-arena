...
export async function middleware(request: NextRequest) {
    ...
    let headerValues = { username: decodedValue.username, email: decodedValue.email, id: decodedValue.id, isPremium: decodedValue.isPremium, role: decodedValue.role };
...
export async function verifyJWT(token: string): Promise<null | { username: string, email: string, id: string, isPremium: boolean, role: string }> {
    ...
    try {
        ...
        return { username: payload.username as string, email: payload.email as string, id: payload.id as string, isPremium: payload.isPremium as boolean, role: payload.role as string };
    } catch (error) {
